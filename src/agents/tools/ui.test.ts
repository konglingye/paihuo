...
describe('ui 工具（reveal_card/notify/open_tool_site）', () => {
  ...
  describe('open_tool_site', () => {
    ...
    it('打开目录里工具的真实 URL，不传 textToCopy 就不碰剪贴板', async () => {
      ...
      const result = await openToolSiteTool.handler({ toolId: 'gamma' });

      expect(createSpy).toHaveBeenCalledWith({ url: expect.stringContaining('gamma.app') });
      expect(navigator.clipboard.writeText).not.toHaveBeenCalled();
      expect(result).toMatchObject({ opened: true, name: 'Gamma' });
      createSpy.mockRestore();
    });

    it('传了 textToCopy 就先复制再开标签', async () => {
      const createSpy = vi.spyOn(browser.tabs, 'create').mockResolvedValue({} as never);
      await openToolSiteTool.handler({ toolId: 'gamma', textToCopy: '帮我出个大纲【产品卖点】' });

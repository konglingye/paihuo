...
describe('get_prompt_template', () => {
  ...
  it('传已知 toolId 时把工具名带进角色段', async () => {
    const result = await getPromptTemplateTool.handler({ taskType: 'slide', toolId: 'kimi' });
    expect(result.role).toContain('Kimi');
  });
...
describe('draft_user_prompt', () => {
  ...
  it('给已存在的任务生成外部提示词', async () => {
    const [task] = useTasksStore.getState().addTasks([
      { title: '发布会 PPT', type: 'slide', fit: 'assist', toolId: 'kimi', saveMin: 90, fragmentId: 'f1' },
    ]);

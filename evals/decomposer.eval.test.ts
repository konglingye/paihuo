...
import { materializeDecomposerOutput } from '@/src/agents/materializeDecomposerOutput';
...
describe('拆解 evals（arch §7）：≥6 段金标准原话，mock 通道验证结构性规则始终成立', () => {
  ...
  describe.each(DECOMPOSER_GOLDENS)('$id — $description', (golden) => {
    it('走完整 runDecompose 链路，契约校验通过且任务数在预期区间内', async () => {
      ...
      // runDecompose 现在不自动落库了（用户要拆解完先确认再入库），evals 关心的是完整链路
      // 落库后的结构，这里手动补上确认之后本该发生的那一步
      const materialized = materializeDecomposerOutput(result.output, result.fragmentId);
      const taskCount = materialized.tasks.length;
    ...
    it('fit=self 的任务必须带非空小抄（prompt）——真实反馈的 bug：模型漏填，卡片「复制小抄」按钮复制出空文本', async () => {
      const fragment: Fragment = {
        id: `eval-${golden.id}-self-prompt`,
        raw: golden.rawText,
        attachments: golden.attachmentText ? [{ name: '附件.txt', text: golden.attachmentText }] : [],
        createdAt: 0,
      };
      ...
      materialized.tasks
        .filter((t) => t.fit === 'self')
        .forEach((t) => {
          expect(t.prompt?.trim().length ?? 0, `任务「${t.title}」fit=self 却没有小抄`).toBeGreaterThan(0);
        });
    });

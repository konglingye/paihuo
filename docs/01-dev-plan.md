...
- [x] **T17 事件编排**：EventBus + 规则表（arch §5）：dump.created→decomposer→organizer 链、task.completed→轻量建议、17:30 alarm→日报提醒；自动 run 禁 ui/external 工具。
  DoD：单测触发规则；mock 下完成任务后出现"建议下一件"。
  ✅ 2026-07-04 验证方式：`src/agents/events.ts`——`EventBus` 类（规则=纯数据数组，按 name 开关，emit 用 `Promise.allSettled` 保证某条规则报错不影响其它规则也不让 emit 本身 reject），6 个用例；`src/agents/eventRules.ts` 的 `registerDefaultEventRules(bus, getDeps)` 用 getDeps 惰性求值（每次触发才读最新 settings，不是注册时就固定死）注册三条规则：①`dump.created`→未完成任务≥2 件才跑一次 organizer（省一次没必要的 LLM 调用），命中的关联落 relationsStore（RelationBanner 自动出现，不用手动点"找关联"）；②`task.completed`→纯数据查找（不调 LLM）：看这件事在 relationsStore 里有没有已知关联，对方还没做完就用 uiStore.notify 弹"建议接着做…"；③`alarm.eod`→查今天有没有完成任务且没写日报，命中则让 worklogStore 记一条待展示的提醒，8 个用例覆盖三条规则的触发/不触发分支；`worklogStore` 加 `eodNudgeDate`/`checkEodAlarm`/`dismissEodNudge`（4 个新用例）；`tasksStore.completeTask` 和 `settingsStore.setSettings` 分别 emit `task.completed`/`settings.changed`（后者目前没有模型探测缓存可失效，机制先接好留空，避免为了失效而发明一个不存在的缓存）；`useDecomposeRun` 拆解成功后 emit `dump.created`；`entrypoints/background.ts` 用 `chrome.alarms.create` 注册每日 17:30 闹钟（需要给 wxt.config.ts 加 `alarms` 权限），触发时先 rehydrate 三个 store 再 emit `alarm.eod`——MV3 两个上下文（sidepanel/background）不共享模块状态，两边各自调用一次 `registerDefaultEventRules` 挂到各自的 `eventBus` 单例上；`entrypoints/sidepanel/App.tsx` 模块顶层注册 sidepanel 侧规则+挂载时把攒下的 eod 提醒转成 toast。Playwright 用真实解包扩展验证：service worker 里 `chrome.alarms.getAll()` 确认 17:30 闹钟已注册；拆解出发布会四任务后关联横幅**自动出现**（没有点"找关联"按钮）；标记"发布会 PPT"完成后**自动弹出**"建议接着做「发布会宣传文案」"提醒；全程控制台无报错，截图存 `docs/qa/T17-*.png`；`pnpm test`（54 文件 373 用例）/`pnpm compile`/`pnpm build`（mock 与非 mock）全绿。**M4 里程碑完成**：盯活闭环（对话+事件自动化）且 agent 全程可见（trace 回放）。
...
- [x] **T20 右键收集与快捷键**：contextMenus 选中文本→倒活框；`Cmd/Ctrl+Shift+Y`。
  DoD：手动验证记录；权限未超集。
  ✅ 2026-07-04 验证方式：新增 `src/store/captureStore.ts`（右键收集中转站，persist 到 chrome.storage——background 的 contextMenus 处理器和 sidepanel 是两个不共享内存的 JS 上下文，选中文本得经这一跳才能到倒活框），`setPendingText`/`takePendingText`（取走即清空，避免重复消费），4 个用例；`entrypoints/background.ts` 注册 `contextMenus.create({contexts:['selection']})`+`onClicked` 监听（拿 `info.selectionText` 写进 captureStore 再 `sidePanel.open`）与 `commands.onCommand`（`toggle-panel`→`sidePanel.open`，Chrome 的 sidePanel API 目前没有程序化"关闭"方法，只能做到"唤起"，这是平台限制不是漏做）；`wxt.config.ts` 加 `manifest.commands.toggle-panel`（`Ctrl+Shift+Y`/`Command+Shift+Y`），不需要新增 `permissions`（`contextMenus` 早就在权限表里，`commands` 声明本身不占权限位）；`App.tsx` 挂载时 rehydrate 一次 captureStore，有待处理文本就切到"活儿" tab，`DumpPanel` 挂载时取走文本填进倒活框（不触发拆解）。手动验证记录：Chrome 的 `contextMenus`/`commands` API 都没有面向扩展开放的"程序化触发 onClicked/onCommand"或"查询已注册项"接口（原生右键菜单点击和全局键盘快捷键都发生在浏览器 chrome 层，不在页面 DOM 里，Playwright 的页面自动化天然够不到）——这是 Chrome 平台的已知限制，DoD 本身也写的是"手动验证记录"而非全自动化。改为 Playwright 真实验证下游逻辑：确认 service worker 启动阶段（含 `contextMenus.create`+`commands` 注册）控制台无报错；模拟右键处理器会做的动作（写 `chrome.storage.local` 的 `paihuo:capture`）后真实打开 sidepanel，确认自动切到"活儿" tab、倒活框正确显示选中文本、且没有触发自动拆解（符合"不自动拆解"要求）；刷新验证同一段文本不会被重复消费；读取真实构建产物 `manifest.json` 逐项核对 `permissions` 数组与 T20 之前完全一致（无超集）且 `commands.toggle-panel` 正确声明。截图存 `docs/qa/T20-*.png`；`pnpm test`（58 文件 400 用例）/`pnpm compile`/`pnpm build`（mock 与非 mock）全绿。
- [x] **T21 打磨**：空态/错误态/429 引导全检、reduced-motion、360-420px 自适应、文案对照原型校对；`docs/qa/checklist.md` 完稿打勾。
  DoD：`docs/qa/checklist.md` 完稿；测试/编译/构建全绿。
  ✅ 2026-07-04 验证方式：全局 `prefers-reduced-motion: reduce` 支持（`src/styles/tokens.css`），排查中定位两个 Tailwind v4 构建坑并记录：自定义 `@keyframes ping` 撞名内置 `animate-ping` 被悄悄覆盖（改名 `orb-ping`）、规则前多行注释导致整条规则被构建产物丢弃（换单行注释），Playwright `emulateMedia({reducedMotion:'reduce'})` A/B 验证动画时长 2.8s→0.00001s 生效；360/420px 视口下总览/活儿/汇报/对话/设置五面板截图核查，无需修复；空态/错误态审计发现并修复两处真实缺口——`runOrganize.ts` 此前把 LLM 真报错（网络/超时/401/429）和"模型正常返回零建议"统一显示成「没找到明显的关联」，用户会把 key 失效误判成活儿真没关联，现在 `RunOrganizeResult.error` 只在真报错时填充，`describeOrganizeError` 给出与其余 run* 文件一致的四态文案，`DumpPanel.handleFindRelations` 优先展示该 error；`ChatSheet` 首次打开零消息时新增居中空态引导（原型没有这个状态，原型抽屉永远预置演示对话，这里是新增不是照抄），可达路径是关联横幅「好，先定关键信息」只 `openChat()` 不发消息；顺带统一了 `SettingsSheet` 429 文案（去掉多余的「稍」字，与另外三处 describeXError 措辞一致）。文案对照原型校对：审查两处疑似差异（原型「剧本 ①/②/③」按钮、「拆出了 4 件事」硬编码数字）均确认是原型自身的演示脚手架或写死的具体数字，不应照抄，现有实现已正确对齐原型语气无需改动。测试新增：`runOrganize.test.ts` 3 个真报错/正常空建议场景断言（顺带修了测试自身 `scriptedDriver` mock 驱动脚本下标未夹住导致重试场景读到 `undefined` 抛出无关 TypeError 掩盖真实断言的 bug）、`ChatSheet.test.tsx` 2 个空态用例；Playwright 全链路验证「试试示例」拆解→「手动找一次关联」出横幅→点「好，先定关键信息」→ 抽屉展开显示空态引导，截图 `docs/qa/T21-chat-empty-state.png`；完整清单见 `docs/qa/checklist.md`。`pnpm test`（58 文件 404 用例）/`pnpm compile`/`pnpm build`（mock 与非 mock）全绿。
- [x] **T22 e2e**：Playwright 加载解包扩展，mock 全链路（配置→倒活→拆解→筛选→对话→汇报→重启数据仍在→trace 有记录）。
  DoD：`pnpm e2e` 一条命令绿。
  ✅ 2026-07-05 验证方式：`playwright.config.ts`（`testDir: e2e`，`trace/video` 关、`screenshot: only-on-failure`）+ `e2e/fixtures.ts`（官方 Chrome 扩展测试模式：`launchPersistentContext` 加载真实解包扩展、从 `serviceWorkers()` 解析 extensionId，`userDataDir` 用 `fs.mkdtempSync` 而非空字符串——排查中发现空字符串在个别环境下会让首个 service worker 事件时序不稳）；`e2e/full-chain.spec.ts` 单测试内用 `test.step` 串联 DoD 要求的全部八段：配置（设置抽屉走完 3 步向导，mock 拦截 `连接并拉取模型列表`/`测试连接`，断言「已连通」）→倒活+拆解（「试试示例」+「拆解」产出发布会四件套卡）→筛选（筛"演示"剩 1 张、切回"全部"复原 4 张）→对话（dock 快捷语句「会议纪要发完了」触发 complete_task，任务卡 checkbox 翻成 aria-checked=true）→汇报（默认日报，生成非空文本）→重启数据仍在（`page.reload()` 后 4 张任务卡与完成态都还在，验证 chrome.storage.local 持久化真实生效）→trace 有记录（跳到 `trace.html`，decomposer/orchestrator/reporter 三个 profileName 均可见）。调试过程中定位到一个真实的测试脆弱点并修复：对话结论文本还在流式吐字（`busy=true`）时就关闭 ChatSheet，会跟"输入框重新可用"的状态更新赛跑，把背景蒙层卡在半开状态挡住后续点击——即使是 5 次里 4 次复现的确定性 bug，也是"点击时机早于 UI 真正稳定"这一类真实场景（不是 Playwright 本身的问题）；修复为显式等输入框 `toBeEnabled()`（busy 落定）后再关，且改点蒙层大命中区而不是 28×28px 的小关闭图标（消息列表重排时坐标点击偶发落空，蒙层点击对布局抖动天然免疫），连续跑 5 次 100% 稳定通过；`vitest.config.ts` 补 `exclude: ['e2e/**']`（否则 vitest 会把 `@playwright/test` 的 `test()` 当自己的测试收集，直接报错）；`.gitignore` 加 `test-results/`/`playwright-report/`。`pnpm test`（58 文件 404 用例）/`pnpm eval`（3 文件 31 用例）/`pnpm compile`/`pnpm build`（mock 与非 mock）/`pnpm e2e` 全绿。
- [x] **T23 打包**：`pnpm zip` 产 Chrome/Edge 包；README 安装图文+用户版「3 步接 AI」指南；v0.1.0。
  DoD：干净实例安装跑通 mock 链路。
  ✅ 2026-07-05 验证方式：`package.json` 版本早已是 `0.1.0`，`pnpm zip`（不设 `VITE_PAIHUO_MOCK`，保证发布物是真实非 mock 构建）产出 `.output/paihuo-0.1.0-chrome.zip`（Chrome/Edge 通用格式，Edge Add-ons 直接吃 Chrome MV3 包）；DoD 的"干净实例安装跑通 mock 链路"单独验证——另跑一次 `VITE_PAIHUO_MOCK=1 pnpm zip`，把产出的 zip 解压到一个全新的临时目录（模拟"刚下载解压，从没跑过"的干净实例），Playwright 用 `fs.mkdtempSync` 出的全新用户配置目录加载这个解压目录（不是 `.output/chrome-mv3` 原始产物，是真的走了一遍"打包→解压→加载"），完整走一遍拆解→对话完成→生成日报→刷新数据仍在，全部通过后清理临时文件，再用真实构建重新生成最终交付用的 zip（验证脚本和临时目录未提交，产物本身在 `.output/` 也不进 git，走 `pnpm zip` 自己产出）；README 新增面向最终用户的「安装」+「接上 AI，只需 3 步」两节，配 5 张 Playwright 真实截图（`docs/install/*.png`，专门开的新目录，跟 `docs/qa/*.png` 的 gitignore 规则区分开、这几张会随仓库分发）：加载成功的扩展列表、总览页首次打开、选平台/粘 key/已连通三步向导；原「开发启动包」内容整体下移，改为「以下内容面向继续开发的人」的第二部分，不影响原有 Goal 协议说明。`pnpm test`（58 文件 404 用例）/`pnpm compile`/`pnpm build`（mock 与非 mock）全绿（本任务未改动应用代码，只涉及打包与文档，因此本轮未重跑 `pnpm e2e`/`pnpm eval`——T22 已在同一份构建产物上验证过，此处的 zip 验证是它的延伸）。
